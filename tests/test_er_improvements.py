...
def main() -> None:
    ...
    from file_profiler.output.er_diagram_writer import _is_audit_fk, generate
    ...
    print(
        f"Reduction: {len(report.candidates)} -> {len(mermaid_lines)} edges "
        f"({100 - len(mermaid_lines) * 100 // max(len(report.candidates), 1)}% reduction)"
    )
